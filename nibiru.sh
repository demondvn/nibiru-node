...
sed -i 's|^pruning *=.*|pruning = "custom"|g'$HOME/.nibid/config/config.toml
sed -i 's|^pruning-keep-recent  *=.*|pruning-keep-recent = "100"|g' $HOME/.nibid/config/config.toml
sed -i 's|^pruning-interval *=.*|pruning-interval = "10"|g' $HOME/.nibid/config/config.toml
sed -i 's|^snapshot-interval *=.*|snapshot-interval = 2000|g'$HOME/.nibid/config/config.toml

sed -i 's|^minimum-gas-prices *=.*|minimum-gas-prices = "0.0001unibi"|g' $HOME/.nibid/config/config.toml
